...
以下选项用于枚举后端 DBMS 信息、架构和表中包含的数据。此外，你可以运行自定义的SQL 语句。

### 获取全部数据

开关：`--all`

该开关用于用户想要通过使用单个开关获取远程所有可访问数据的情况。但不建议这么做，因为它会产生大量的请求去获取有用和无用的数据。

### Banner

开关：`-b` 或 `--banner`

大多数现代 DBMS 具有一个函数和/或环境变量，它会返回 DBMS 版本，并最终在其补丁级别详细介绍底层系统。通常这个函数是 `version()` ，环境变量是 `@@version`，这取决于目标 DBMS。
...
使用此开关可以从 Web 应用程序获取 DBMS 当前用户，该用户可以有效对后端 DBMS 执行查询。
