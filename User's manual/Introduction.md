...
页面显示跟原来的不一样（AND+1=2 条件取值为**False**）。这可能意味着`index.php`的`id` `GET` 参数存在 SQL 注入漏洞。进一步推断，这种情形表明用户输入的数据被直接用于构造SQL 语句传送到数据库，并没有被过滤。

这种设计缺陷在动态网页应用中十分常见，此类型漏洞与后端数据管理系统或后端编程语言并没有关系，漏洞的引入通常存在于代码的编写逻辑里面。 从2013年开始, [开放Web应用安全组织](http://www.owasp.org)已将此类漏洞列入[最常见](https://owasptop10.googlecode.com/files/OWASP%20Top%2010%20-%202013.pdf)严重Web应用漏洞的前十名单。

从上面的例子我们发现了存在可以利用的参数，现在我们可以通过在每一次的 HTTP 请求中修改`id`进行相关的漏洞检测。
重回上面的情景，通过相关的经验，我们可以对`get_ini.php`页面中如何使用用户提供的参数构建出相关的`SELECT`语句做一个大致的猜测。参考下面的 PHP 伪daima:
```
